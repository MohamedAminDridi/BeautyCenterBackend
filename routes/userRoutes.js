...
// GET /me (authenticated user data) - CORRECTED
router.get('/me', authMiddleware, async (req, res) => {
  try {
    // UPDATED: Added scheduleOverrides to the select() statement
...
    }
    // SIMPLIFIED & CORRECTED: Sending the user object directly is cleaner and ensures all selected fields are included.
    res.json(user);
  } catch (err) {
...
    if (req.user.role !== 'personnel') {
        return res.status(403).json({ message: 'Forbidden: Only personnel can set overrides.' });
    }
